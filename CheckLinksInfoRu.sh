...
#	s3=`echo "$xmlbuf"| sed "$y!d" |perl -e 'while(<>) { m/(\w+).+?(\w+)/;print "$2\n"; }'` # CDR
	...
	echo $s2 "" $s4 "" $s5 "" $s6 >> /tmp/arrline
#echo "3"	
done
echo "3"
#       array1=( `cat /tmp/arrline |awk '{print $1}'`)
       array2=( `cat /tmp/arrline |awk '{print $1}'`)
#       array3=( `cat /tmp/arrline |awk '{print $3}'`)
       array4=( `cat /tmp/arrline |awk '{print $2}'`)
       array5=( `cat /tmp/arrline |awk '{print $3}'`)
       array6=( `cat /tmp/arrline |awk '{print $4}'`)
